...
class TestCheckDependancies:
    def test_detects_percent_macro_call(self, tmp_path):
        ...
        result = check_dependancies(f, ["shared"])
...
class TestProcessRepository:
    ...
    def test_oracle_calls_cross_repo_uses_dependency_names_not_chars(self, tmp_path):
        """
        Bug #7: 'Oracle Calls' iterates over the dependency string character by
        character instead of splitting it into names. This test will fail until
        that bug is fixed, and serves as a regression guard once it is.
        """
        ...
        df = process_repository(main_dir)
        # main.sas depends on oracle_macro, which has Oracle calls
        # The Oracle Calls flag should be True — currently broken because
        # dependencies is iterated as a string of chars, not split into names
        row = df[df["File Name"] == "main.sas"]
        assert not row.empty
        # Uncomment the assertion below once bug #7 is fixed:
